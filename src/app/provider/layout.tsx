...
import {
  LayoutDashboard, Briefcase, User, LogOut, X, Sun, Moon,
} from 'lucide-react'
      ...
      >
        <div className="flex items-center justify-between px-6 pt-6 pb-5">
          <Link href="/" aria-label="Home" className="flex items-center">
            <img src={theme === 'dark' ? '/logo.svg' : '/footer-logo.svg'} alt="Everiithing" className="h-8 w-auto" />
          </Link>
          <button onClick={() => setMobileOpen(false)} className="md:hidden bg-transparent border-none p-1 cursor-pointer" aria-label="Close menu">
        ...
        <div className="p-4 space-y-1">
          <button
            onClick={toggleTheme}
            className="flex w-full items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ease-out lg:hidden"
          ...
          <Link
            href="/"
            className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ease-out text-[var(--color-onSurfaceVariant)] hover:text-[var(--color-error)]"
          >
      ...
      <main className="lg:ml-[260px] min-h-screen p-4 md:p-6 lg:p-8">
        <div className="max-w-7xl mx-auto">
          <TopBar
            ...
            profileHref="/provider/dashboard/profile"
            theme={theme}
